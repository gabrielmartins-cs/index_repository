...
class WordxDossieController extends Controller
{
	...
	public function criarWordx($idarquivodocx, $textojson, $teses, $dadosXmlArteria, $nomedapasta, $sintesefatica, $paragrafosConclusao)
	{
		...
		// 1. Separa as teses em "gavetas" (agrupamento)
		$tesesAgrupadas = [
			'preliminares' => [],
			'prejudiciais' => [],
			'merito'       => []
		...
		foreach ($teses as $tese) {
			// Caso alguma tese venha sem categoria, jogamos para o mérito por segurança
			$categoria = $tese->categoria ?? 'merito';
			$tesesAgrupadas[$categoria][] = $tese;
		}

		// 2. Define a ordem de impressão e os títulos dos grupos
		$ordemCategorias = [
		...
		// 3. Array auxiliar para converter números comuns em Romanos
		$romanos = [
			1 => 'I',
			2 => 'II',
			3 => 'III',
		...
		// 4. Inicia o Loop Principal passando pelos grupos (Preliminares -> Prejudiciais -> Mérito)
		foreach ($ordemCategorias as $chaveCategoria => $tituloCategoria) {

			// Se não houver nenhuma tese para essa categoria (ex: não tem preliminar), pula e não imprime nada
			if (empty($tesesAgrupadas[$chaveCategoria])) {
				continue;
			}

			// Pega o número romano do Tópico Principal (Ex: I, II, III)
			$numRomanoGrupo = $romanos[$grupoContador] ?? $grupoContador;

			// === IMPRIME O TÍTULO PRINCIPAL DO GRUPO (Ex: I. DAS PRELIMINARES) ===
			$section->addText(
			...
			// Reseta o contador de subtópicos sempre que entra em um novo grupo principal
			$subContador = 1;

			// 5. Entra no loop das teses pertencentes APENAS a este grupo
			foreach ($tesesAgrupadas[$chaveCategoria] as $tese) {
				...
				if (isset($tese->tipo) && $tese->tipo === 'citacao') {
					...
					// === MONTA O NÚMERO DO SUBTÓPICO (Ex: III.I, III.II) ===
					$numRomanoSub = $romanos[$subContador] ?? $subContador;
					$numeracaoFinal = $numRomanoGrupo . "." . $numRomanoSub;

					// Imprime com um "Tab" (\t) para dar um pequeno espaço entre o número e o texto
					...
					// Incrementa o contador de subtópico apenas quando imprime um título de tese
					$subContador++;
					...
					if (!empty($tese->conteudo)) {
						// Verifica se a string contém tags HTML (geradas pelo Quill)
						if (strip_tags($tese->conteudo) !== $tese->conteudo) {

							// 1. O Quill separa as linhas usando tags <p>. Vamos capturar o conteúdo de dentro delas.
							// Isso evita que o PHPWord crie quebras de linha com estilos errados.
							preg_match_all('/<p[^>]*>(.*?)<\/p>/is', $tese->conteudo, $matches);

							if (!empty($matches[1])) {
								$paragrafosHtml = $matches[1];
							} else {
								// Fallback de segurança se o HTML vier sem tag <p>
								$paragrafosHtml = explode("\n", $tese->conteudo);
							}

							// 2. Definimos o estilo rígido do parágrafo (SOLUÇÃO DOS PROBLEMAS 1 e 2)
							$estiloParagrafoABNT = [
								'alignment'   => \PhpOffice\PhpWord\SimpleType\Jc::BOTH, // Texto Justificado
								'lineHeight'  => 1.5,                                    // Espaçamento de 1,5
								'spaceAfter'  => 120,                                    // Espaço após o parágrafo
								'indentation' => ['firstLine' => 720]                    // O famoso recuo (\t) da primeira linha
							];

							// 3. Lemos parágrafo por parágrafo
							foreach ($paragrafosHtml as $htmlDoParagrafo) {
								// Limpa para verificar se não é um parágrafo vazio
								$htmlLimpo = trim(strip_tags($htmlDoParagrafo));

								if ($htmlLimpo !== '') {
									// Cria a "linha" no documento que respeitará o espaçamento 1.5 e justificado
									$textRun = $section->addTextRun($estiloParagrafoABNT);

									$textRun->addText("\t", ['name' => 'Tahoma', 'size' => 12, 'color' => '1B2232']);

									// Envelopamos o conteúdo para manter a fonte Tahoma.
									$htmlComFonte = '<span style="font-family: Tahoma; font-size: 12pt; color: #1B2232;">' . trim($htmlDoParagrafo) . '</span>';

									// Injeta o HTML formatado logo após o TAB
									\PhpOffice\PhpWord\Shared\Html::addHtml($textRun, $htmlComFonte, false, false);
								}
							}
						} else {
							// É texto simples antigo (Fallback para teses salvas antes da mudança do sistema)
							$paragrafos = explode("\n", $tese->conteudo);
							foreach ($paragrafos as $paragrafo) {
								...
								if ($paragrafoLimpo !== '') {
									...
									$section->addText("\t" . $paragrafoLimpo, $estiloTexto, [
										...
										'lineHeight' => 1.5,
										'spaceAfter' => 120,
										'indentation' => ['firstLine' => 720] // Mantido o recuo também para os textos antigos
									]);
								}
							}
						}
			...
			// Só incrementa o contador do grupo (I para II) depois que finalizar TODAS as teses daquela categoria
			$grupoContador++;
		}
		// =========================================================================
		// === TÓPICO DA CONCLUSÃO ===
